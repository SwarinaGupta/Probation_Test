...
public class Statistics 
{
	    ...
	    public int getTotalNumberOfItems() {
	        int totalItems = 0;
	        //Using sql query to get the total number of items
	    ...
	    public List<Integer> getCategoryItemCounts() {
	        ...
	        //Using the joins, so we can get items on the basis of the category Id
	        String sql = "SELECT COUNT(items.id) " + "FROM categories LEFT JOIN items ON categories.id = items.category_id " +"GROUP BY categories.name";
	        try (
